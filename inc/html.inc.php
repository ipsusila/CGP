	...
	foreach($hosts as $host) {
		...
		if ($CONFIG['showload']) {
			...
			if ($rrd_info &&
			 ...
			 //for percent
            $rrd_info = $rrd->rrd_info($CONFIG['datadir'].'/'.$host.'/load/percent.rrd');
            $info = 'ds[value].last_ds';
            if ($rrd_info && isset($rrd_info[$info])) {
                $percent_load = $rrd_info[$info];
                $class = '';
                if ($percent_load > 90)
                    $class = ' crit';
                else if ($percent_load > 70)
                    $class = ' warn';

                printf('<div class="field%s">%d%%</div>', $class, $percent_load);
            }
		}
		...
		if ($CONFIG['showmem']) {
			...
			} else if ($rrd_info_mu && $rrd_info_mf) {
				$info='ds[value].last_ds';
				if (isset($rrd_info_mu[$info]) && isset($rrd_info_mf[$info]) ) {
					$percent_mem =	$rrd_info_mu[$info] * 100 / ($rrd_info_mu[$info] + $rrd_info_mf[$info]);
				}
			}
		...
		if ($CONFIG['showtime']) {
			//$rrd_info = $rrd->rrd_info($CONFIG['datadir'].'/'.$host.'/load/load.rrd');
			//load.rrd may not exists, e.g. when load generated by CollectM
			//Instead of load, use memory-free.rrd
			$rrd_info = $rrd->rrd_info($CONFIG['datadir'].'/'.$host.'/memory/memory-free.rrd');
			if ($rrd_info) {
